...
<head>
    ...
    $socialIcons = ['linkedin'=>'fa-linkedin','github'=>'fa-github','facebook'=>'fa-facebook',
                    ...
                    'tiktok'=>'fa-tiktok','behance'=>'fa-behance','dribbble'=>'fa-dribbble',
                    'medium'=>'fa-medium','stackoverflow'=>'fa-stack-overflow','website'=>'fa-globe'];
    ?>
    <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    /* experience */
    .exp-item{padding:14px 0;border-bottom:1px solid #f0f0f0;display:flex;gap:14px}
    .exp-item:last-child{border-bottom:none;padding-bottom:0}
    .exp-icon{width:40px;height:40px;border-radius:10px;background:rgba(<?= $accentRgb ?>,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0}
    .exp-icon i{color:<?= e($accent) ?>;font-size:16px}
    .exp-org{font-size:14px;font-weight:700;color:#1a1a1a;margin-bottom:2px}
    .exp-title{font-size:13px;color:<?= e($accent) ?>;font-weight:600;margin-bottom:2px}
    .exp-meta{font-size:12px;color:#999;margin-bottom:4px}
    .exp-desc{font-size:13px;color:#555;line-height:1.6}
    .exp-badge{display:inline-block;padding:2px 8px;background:rgba(<?= $accentRgb ?>,0.1);color:<?= e($accent) ?>;border-radius:4px;font-size:11px;font-weight:600;margin-left:6px}
    .exp-current{background:rgba(34,197,94,.12);color:#16a34a}
    ...
    /* Powered-by footer */
    .pub-powered{text-align:center;padding:28px 20px 20px;border-top:1px solid #f0f0f0;margin-top:8px}
    .pub-powered-inner{display:inline-flex;align-items:center;gap:10px;padding:10px 20px;background:#fafafa;border:1px solid #eee;border-radius:40px}
    .pub-powered-inner span{font-size:11px;color:#bbb;text-transform:uppercase;letter-spacing:.07em;font-weight:600}
    .pub-powered-logo{display:flex;align-items:center;gap:6px;text-decoration:none}
    .pub-powered-logo img{height:18px;object-fit:contain}
    .pub-powered-logo strong{font-size:14px;font-weight:700;color:#1a6b4a}
...
<div class="pub-card">
    ...
    <div class="pub-main">
        ...
        <?php if (!empty($profile['languages']) && vis($vis,'languages',true)): ?>
        <div class="pub-section">
            <div class="pub-sec-head"><i class="fa-solid fa-language"></i><h3>Languages</h3></div>
            <div class="pub-sec-body">
                <div class="skill-tags">
                    <?php foreach (array_map('trim', explode(',', $profile['languages'])) as $lang):
                        if (!$lang) continue; ?>
                    <span class="skill-tag" style="background:rgba(<?= $accentRgb ?>,0.05)"><?= e($lang) ?></span>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
        ...
        <?php if (!empty($profile['hobbies']) && vis($vis,'hobbies',true)): ?>
        <div class="pub-section">
            <div class="pub-sec-head"><i class="fa-solid fa-heart"></i><h3>Interests &amp; Hobbies</h3></div>
            <div class="pub-sec-body">
                <div class="skill-tags">
                    <?php foreach (array_map('trim', explode(',', $profile['hobbies'])) as $hobby):
                        if (!$hobby) continue; ?>
                    <span class="skill-tag" style="background:rgba(<?= $accentRgb ?>,0.05)"><?= e($hobby) ?></span>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
        ...
        <?php
        // Experience from user_experience table (detailed, repeatable)
        $hasExpSection = !empty($experience) && vis($vis,'experience',true);
        // Legacy: single business association
        $hasLegacyBiz  = !$hasExpSection && $business && vis($vis,'business',true);

        if ($hasExpSection): ?>
        <div class="pub-section">
            <div class="pub-sec-head"><i class="fa-solid fa-briefcase"></i><h3>Work Experience</h3></div>
            <div class="pub-sec-body" style="padding:8px 18px">
                <?php foreach ($experience as $exp):
                    $startFmt = !empty($exp['start_date']) ? date('M Y', strtotime($exp['start_date'])) : '';
                    $endFmt   = $exp['is_current'] ? 'Present' : (!empty($exp['end_date']) ? date('M Y', strtotime($exp['end_date'])) : '');
                    $typeLabel = $empTypeLabels[$exp['employment_type'] ?? ''] ?? '';
                ?>
                <div class="exp-item">
                    <div class="exp-icon"><i class="fa-solid fa-building"></i></div>
                    <div style="flex:1">
                        <div class="exp-org"><?= e($exp['organisation']) ?>
                            <?php if ($typeLabel): ?><span class="exp-badge"><?= e($typeLabel) ?></span><?php endif; ?>
                            <?php if ($exp['is_current']): ?><span class="exp-badge exp-current">Current</span><?php endif; ?>
                        </div>
                        <div class="exp-title"><?= e($exp['job_title']) ?></div>
                        <div class="exp-meta">
                            <?= $startFmt ? e($startFmt) . ($endFmt ? ' – ' . e($endFmt) : '') : '' ?>
                            <?php if (!empty($exp['location'])): ?> &nbsp;·&nbsp; <?= e($exp['location']) ?><?php endif; ?>
                        </div>
                ...
                <div class="edu-item">
                    ...
                    <div>
                        ...
                        <div class="edu-years">
                            ...
                            — <?= $edu['is_current'] ? '<span style="color:'.e($accent).';font-weight:600">Present</span>' : ($edu['to_year'] ? e($edu['to_year']) : '?') ?>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
    ...
    <!-- Powered by Byabsayee branding -->
    <div class="pub-powered">
        <div class="pub-powered-inner">
            <span>Powered by</span>
            <a href="/" class="pub-powered-logo">
                <img src="/assets/images/ByabsayeeLogo.png" alt="Byabsayee" onerror="this.style.display='none'">
                <strong>Byabsayee</strong>
            </a>
        </div>
    </div>
